...
- 최종 확인된 Lab controller, client-facing proxy, launcher/config의 SHA-256은 다음과 같다.
  값은 내용이 아니라 설치된 파일의 식별자다.

  - `labctl`: `3aff578b0a7991220bd47c97b3026264e46b3dde666a5a1832e97eefa430cc95`
  - `bin/lab-proxy.js`: `7f2a52db0ac0a859dd7004c8f646c729c0365c12094f6bbd6389da8d96cb4ddf`
...
OpenCode v2.0.3 upstream은 Galaxy Tab loopback `127.0.0.1:4098`에만
listen한다. 이 버전은 `serve`가 설정되지 않은 경우에도 임의 Basic Auth를
생성하므로, `state/server-password`를 Lab 내부 upstream 호환 경계로만
사용한다. `bin/lab-proxy.js`는 같은 Lab root 안에서 `127.0.0.1:4097`에
listen하고 upstream 인증을 주입한다. 따라서 Mac, Desktop, Tailscale, iPhone이
보는 client-facing Lab endpoint에는 Basic Auth prompt가 없다. 이 프록시는
Mac이나 기존 프로젝트·credential을 읽지 않는다.
...
`https://hangyu-macmini.tailf756fe.ts.net/`로 proxy한다. 이것이 유일한
tailnet 원격 접근 경계이며, Funnel은 활성화하지 않고 Galaxy Tab 주소를 직접
공개하지 않는다. Mac의 local tunnel과 Tailscale URL에서 client-facing 무인
health/Web root를 확인했다. iPhone Safari는 같은 tailnet URL을 사용하며
Tailscale tailnet membership/ACL이 접근 권위다. 현재 자동화 범위에는 iPhone
화면 자체의 관찰이 없으므로 device-level UI 성공을 별도로 승격하지 않는다.

OpenCode Desktop의 `Galaxy Tab OpenCode Lab` server entry는
`http://127.0.0.1:18497`만 저장하고 username/password 필드는 비운 상태로
재확인했다. Desktop은 연결된 remote server의 workspace를 새 session project
picker에 노출하지 않아 task 생성 lane은 `pilot`/`blocked`로 남긴다. 실제 task
smoke는 기존 local/remote project를 잘못 선택하지 않도록 Chrome Web client의
Lab workspace에서 수행했고, Chrome Web session은 `verified` 실행 결과다.
...
OpenCode v2.0.3 upstream은 `OPENCODE_SERVER_USERNAME=opencode`와 Lab-local
`state/server-password`로 내부 Basic Auth를 구성한다. 이 인증은 Tab loopback
`4098`과 Lab-local proxy 사이에서만 사용하며 Mac tunnel, Tailscale Serve,
Desktop, iPhone으로 전파하지 않는다. Client-facing endpoint의 health는
무인으로 확인한다. Mac/Desktop, Codex, Console credential을 재사용하거나
문서에 password를 기록하지 않는다.
...
`status`, `health`, and Lab-scoped `exec`. `start`는 upstream server와 proxy를
하나의 Lab process-group으로 기록하고 client-facing `/api/health`를 확인한다.
`stop`은
...
server/proxy group만 확인·재시작하고, model/session/tool/실험을 자동 시작하지
않으며 `termux-wake-lock`도 사용하지 않는다. 명시적 `labctl stop` 뒤에는
marker가 남아 supervisor가 자원을 다시 점유하지 않는다. 수동 `labctl start`가
marker를 지우고 server/proxy를 복구한다.
...
4. 같은 Web session에 synthetic shell/file prompt를 보내 shell 한 번으로
   ...
   재확인했다.
5. `stop` 뒤 Lab process가 없고 `127.0.0.1:4097`과 내부 `4098`이 닫힌 것을
   확인했다. `start` 뒤 새 process-group, client-facing 무인 health, 기존 session metadata와
...
7. Tab client-facing proxy `/api/health`, Mac SSH tunnel
   `http://127.0.0.1:18497/api/health`, Tailscale URL을 각각 credential 없이
   확인했고 모두 `200 application/json`을 반환했다. 같은 세 Web root도
   `200 text/html`과 `<title>OpenCode`를 반환했다. 내부 upstream
   `127.0.0.1:4098`은 credential 없이 `401`이지만 Tab loopback에만 남아
   있으며 외부 endpoint로 라우팅되지 않는다. Tailscale CLI의 Serve와 Funnel
   상태는 모두 `tailnet only`로 표시되어 public Funnel은 사용하지 않았다.
8. Termux:Boot entry를 재실행해 supervisor를 session-independent PPID 1로
   올렸고, Lab server/proxy group만 종료·재시작했다. 명시적 `stop` 뒤에는
   `status=stopped`, marker `disabled`, ports `4097`/`4098` closed가 확인됐고,
   `start` 뒤 client-facing 무인 health와 내부 upstream 인증 경계가 복구됐다.
9. idle 관찰에서 supervisor와 server/proxy는 저부하로 유지됐고, 자동
