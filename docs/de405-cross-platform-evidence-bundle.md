...
The later native x64 legacy matrix run `30768814210` is a separate canonical-v2 final-result evidence layer. It used native x64 Ubuntu glibc/GCC plus native x64 Alpine musl GCC/Clang from one frozen image filesystem and found the same result SHA-256, with zero differing rows/components in all three pairs. It does not alter this diagnostic bundle or promote its historical shadow mismatches to a platform claim; see `docs/de405-historical-reconciliation.md` and `docs/de405-legacy-native-cross-environment-summary.json`.
