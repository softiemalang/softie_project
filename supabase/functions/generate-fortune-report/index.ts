...
- 과장하거나 겁주지 말고, 너무 시적이거나 추상적인 표현으로 흐르지 마세요.

[섹션별 행동 언어]
- love: 기다리기, 말 고르기, 반응 늦추기, 편안한 대화, 기대 낮추기 쪽을 우선 쓰세요.
- relationships: 문장 고르기, 말의 온도 낮추기, 거리 조절, 바로 답하지 않기, 작은 오해 풀기 쪽을 우선 쓰세요.
- work: 마감선 긋기, 결과물 묶기, 우선순위 좁히기, 기준 정하기, 하나만 끝내기 쪽을 우선 쓰세요.
- money: 금액 확인, 결제 미루기, 조건 비교하기, 지출 이유 적기, 남는 부담 보기 쪽을 우선 쓰세요.
- health: 자극 줄이기, 호흡 낮추기, 몸 풀기, 쉬는 시간 확보, 수면/긴장 관리 쪽을 우선 쓰세요.
- mind: 생각 분리하기, 이름 붙이기, 한 줄 메모, 감정과 사실 구분, 머릿속 목록 줄이기 쪽을 우선 쓰세요.
- 한 섹션의 행동어를 다른 섹션에 반복해서 옮겨 쓰지 마세요. 특히 money의 "확인"을 relationships/love/mind의 기본 행동으로 쓰지 마세요.

[반복 표현 제한]
- "확인"은 전체 리포트에서 최대 2개 섹션까지만 쓰세요.
- "정리"는 전체 리포트에서 최대 2개 섹션까지만 쓰세요.
- "부담"은 전체 리포트에서 최대 2개 섹션까지만 쓰세요.
- "천천히", "한 박자", "속도" 계열 표현은 전체에서 과사용하지 말고, 필요하면 love/relationships 중 한 곳에만 가깝게 쓰세요.
- summary, cautions, action_tip까지 포함해 같은 핵심어가 여러 섹션에 줄줄이 반복되면 다른 행동어로 바꾸세요.
...
[action_tip]
- mind/recovery가 강하면 한 줄 메모, 감정과 사실 구분, 머릿속 목록 줄이기처럼 쓰세요.
- communication/relationship가 강하면 말 고르기, 바로 답하지 않기, 말의 온도 낮추기처럼 쓰세요.
- money/work가 강하면 금액 1건 보기, 조건 비교하기, 마감선 긋기, 결과물 하나 묶기처럼 쓰세요.
- health가 가장 강한 날이 아니면 식사나 스트레칭을 기본값으로 쓰지 마세요.
- action_tip은 1~3분 안에 할 수 있는 구체 행동이어야 합니다. "마음을 정리하고 하루를 차분히 보내세요"처럼 넓은 문장은 쓰지 마세요.

[짧은 예시]
- health: "긴장이 몸에 남기 쉬운 날이에요. 오늘은 알림과 자극을 조금 줄이고, 쉬는 시간을 먼저 확보해보세요."
- caution: "민감한 반응이 올라올 땐 바로 말하기보다 한 번 숨을 고르세요."
- caution: "계획 없는 지출은 결제 전에 한 번 더 살펴보세요."
- action_tip: "보내려는 답장 한 문장을 골라 표현의 온도만 낮춰 다시 써보세요."
...
- love, relationships, work, money, health, mind가 서로 다른 행동 축으로 구분되는지 확인하세요.
    ...
    const getFallback = (reason, error = null, status = null, body_preview = null) => {
      ...
      return {
        ...
        content: {
          ...
          cautions: ["민감한 말은 바로 보내지 말고 표현의 온도만 낮춰 다시 써보세요.", "계획 없는 결제는 조건을 비교한 뒤 결정하세요."],
          action_tip: "오늘 꼭 볼 지출 1건만 골라 금액과 이유를 1분 안에 적어보세요.",
          debug
        }
      };
