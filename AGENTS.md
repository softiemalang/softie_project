...
- The agent may create the smallest necessary research record when that record is within the stated task scope. When one meaningful work unit is complete and judged worth preserving as an independent checkpoint, the agent should verify only its related changes and autonomously complete one atomic local commit containing only that checkpoint. Push remains subject to explicit authorization; existing evidence, safety, dirty-work, and file-scoping boundaries remain in force, so in-progress or unrelated dirty work must not be included and a clear question alone does not authorize unrelated publication or remote changes.
