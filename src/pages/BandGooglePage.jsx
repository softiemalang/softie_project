...
export default function BandGooglePage() {
  ...
  async function enterRoom(roomId, memberId = null) {
    ...
    setIsBusy(false)
    if (loaded) setStatus(`${loaded.roomRow.name} 방에 들어왔어요.`)
  }
  ...
  if (!room || !member) {
    return (
      <div className="app-shell band-google-shell">
        <section className="card band-account-card">
        ...
        <section className="card primary-home-card">
          ...
          {myRooms.length === 0 ? (
            <p className="subtle">아직 참여 중인 방이 없어요. 새 방을 만들거나 코드를 받아 참여하면 돼요.</p>
          ) : (
        ...
        <section className="card secondary-card">
          ...
          <div className="field-grid band-two-field-grid">
        ...
        <section className="card secondary-card">
          <p className="section-kicker">방 코드로 참여하기</p>
          <div className="field-grid band-two-field-grid">
  ...
  return (
    <div className="app-shell band-google-shell">
      <section className="card room-hero-card">
        ...
        <div className="action-row band-room-actions">
          <button type="button" className="soft-button" onClick={leaveRoomView}>
            방 목록
          </button>
          <button type="button" className="soft-button" onClick={() => navigator.clipboard?.writeText(room.room_code)}>
            코드 복사
          </button>
        </div>
      </section>
      ...
      <section className="card secondary-card">
        ...
        <div className="inline-form">
          ...
          <button type="button" disabled={isBusy} onClick={updateMemberProfile}>
            이름 저장
          </button>
        </div>
      </section>
      ...
      {isOwner && (
        <section className="card danger-card">
          ...
          <button type="button" className="danger-button" disabled={isBusy || deleteRoomName !== room.name} onClick={deleteRoom}>
            방 삭제
          </button>
        </section>
      )}
