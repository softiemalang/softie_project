...
export const EPHEMERIS_SCHEMA_VERSION = 'astrology-ephemeris-core-v0'
export const RAW_CHART_SCHEMA_VERSION = 'astrology-raw-chart-v0'
...
function precessionMatrix(jdTt) {
  ...
  const gamb = polynomial([-0.052928, 10.556378, 0.4932044, -0.00031238, -0.000002788, 0.0000000260], t) * ARCSEC_TO_RAD
  const phib = polynomial([84381.412819, -46.811016, 0.0511268, 0.00053289, -0.000000440, -0.0000000176], t) * ARCSEC_TO_RAD
  const psib = polynomial([-0.041775, 5038.481484, 1.5584175, -0.00018522, -0.000026452, -0.0000000148], t) * ARCSEC_TO_RAD
  ...
  const matrix = multiply3(multiply3(multiply3(rx(-phib), rz(-psib)), rx(epsa)), rz(gamb))
  return { matrix, t, angles: { gamb, phib, psib } }
}
...
function convertState(state, jdTt) {
  ...
  const { matrix } = precessionMatrix(jdTt)
  const positionEquatorial = apply(matrix, state.slice(0, 3))
  const velocityEquatorial = apply(matrix, state.slice(3, 6))
  ...
  const velocity = [velocityEquatorial[0], ce * velocityEquatorial[1] + se * velocityEquatorial[2], -se * velocityEquatorial[1] + ce * velocityEquatorial[2]]
  ...
  return { longitude, speed, positionKm: position, velocityKmPerSecond: velocity }
}
...
export function composeAstrologyRawChart({ timeAngleInput, tdbMinusTtSeconds, evaluateStates, kernelProvenance = {} } = {}) {
  ...
  return {
    ...
    provenance: {
      ...
      transform: { model: 'iau2006_fukushima_williams_precession_plus_mean_obliquity', input: 'J2000/ICRF_mean_equator', output: 'mean_ecliptic_and_equinox_of_date', speed: 'instantaneous_state_vector_projection' },
    },
  }
}
