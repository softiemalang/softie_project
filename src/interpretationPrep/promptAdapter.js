...
function buildSystemInstruction({ calculationConfidence, interpretationWarnings }) {
  ...
  return [
    ...
    '2. [candidateFacts] 및 [uncertainFactors]에 분기 항목이 존재하는 경우, 단일 명식으로 확정 짓지 말고 "후보별 가능성을 비교하여 조건별 분기"로 나누어 설명한다.',
    ...
    '5. 공통 바탕(Consensus)은 확실한 삶의 지향점으로 설명하되, 변동 요소(Uncertainty)는 "조건별 가정적 가능성 및 대비되는 경향"을 함께 제시하여 포괄적 이해를 돕는다.',
    isLowConfidence ? '6. [HIGH PRIORITY] 본 입력은 신뢰도(Confidence)가 낮거나(Low) 후보 분기가 존재하므로 모든 해석에서 확정적 표현을 절대 금하며, 후보 간 비교와 가능성 제시 위주로 서술한다.' : '',
    needsVerification ? '7. [HIGH PRIORITY] 표준시/지역보정/역사적 검증이 필요한 구간(needs_verification)이므로 산출 결과의 가변성을 분명히 안내한다.' : '',
  ].filter(Boolean).join('\n')
}
