...
function buildSystemInstruction({ calculationConfidence, interpretationWarnings }) {
  ...
  return [
    ...
    isLowConfidence ? '4. [HIGH PRIORITY] 본 입력은 신뢰도(Confidence)가 낮거나(Low) 후보 분기가 존재하므로 모든 해석에서 확정적 표현을 절대 금하며, 후보 간 비교와 가능성 제시 위주로 서술한다.' : '',
    needsVerification ? '5. [HIGH PRIORITY] 표준시/지역보정/역사적 검증이 필요한 구간(needs_verification)이므로 산출 결과의 가변성을 분명히 안내한다.' : '',
  ].filter(Boolean).join('\n')
}
