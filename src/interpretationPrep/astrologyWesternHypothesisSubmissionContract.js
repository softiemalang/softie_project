...
import { validateAstrologyWesternConversationalHandoffPackage } from './astrologyWesternConversationalHandoffPackage.js'

/**
 * Validation contract for a hypothesis submitted by an external
 * conversational consumer.  The engine validates references and boundaries;
 * it never generates, stores, classifies, or personalizes the submission.
 */
export const ASTROLOGY_WESTERN_HYPOTHESIS_SUBMISSION_SCHEMA = 'astrology-western-hypothesis-submission-v0'
export const ASTROLOGY_WESTERN_HYPOTHESIS_SUBMISSION_VERSION = '0.1.0'
export const ASTROLOGY_WESTERN_HYPOTHESIS_SUBMISSION_KIND = 'external_astrology_western_hypothesis_submission'

export const ASTROLOGY_WESTERN_HYPOTHESIS_VALIDATION_SCHEMA = 'astrology-western-hypothesis-submission-validation-v0'
export const ASTROLOGY_WESTERN_HYPOTHESIS_VALIDATION_VERSION = '0.1.0'
export const ASTROLOGY_WESTERN_HYPOTHESIS_VALIDATION_KIND = 'validated_astrology_western_hypothesis_submission'
...
const ALLOWED_SEMANTIC_ROLES = new Set(['source_semantic_result', 'source_local_composition', 'source_lexicon_entry'])
...
const SUBMISSION_CONTRACT = Object.freeze({
  schemaVersion: ASTROLOGY_WESTERN_HYPOTHESIS_SUBMISSION_SCHEMA,
  version: ASTROLOGY_WESTERN_HYPOTHESIS_SUBMISSION_VERSION,
  kind: 'astrology_western_hypothesis_submission_contract',
  input: {
    userContext: {
      status: ['not_provided', 'reported'],
      role: 'reported_context_only_not_fact_or_source_validation',
    ...
    hypothesis: {
      ...
      sourceScope: 'exact_referenced_source_and_single_lineage_scope',
...
function validateHypothesisShape(hypothesis, errors) {
  ...
  if (lineages.length > 1) addError(errors, 'hypothesis:multiple_lineages_forbidden')
...
function validateUserContext(userContext, hypothesisId, errors) {
  ...
  if (status === 'not_provided' && userContext.entries.length > 0) addError(errors, 'user_context:not_provided_has_entries')
  if (status === 'reported' && userContext.entries.length === 0) addError(errors, 'user_context:reported_entries_missing')
  const ids = new Set()
  ...
  return { status, entries: userContext.entries }
}
...
function evidenceState(item) {
  return UNSAFE_EVIDENCE_STATUSES.has(item?.status)
}
...
function inspectEvidence({ handoffPackage, hypothesis }, errors) {
  ...
  for (const item of baseFacts) {
    for (const factRef of item.factRefs || []) if (!declaredFactRefs.has(factRef)) addError(errors, `hypothesis:base_fact_ref_not_declared:${factRef}`)
  }
  ...
  for (const item of sourceEvidence) {
    ...
    if (refs.sourceIds.length === 0 || refs.lineageIds.length === 0 || refs.locatorIds.length === 0) addError(errors, `hypothesis:source_refs_missing:${item.id}`)
    for (const sourceId of refs.sourceIds) if (!sourceIds.includes(sourceId)) sourceIds.push(sourceId)
    for (const lineageId of refs.lineageIds) if (!lineages.includes(lineageId)) lineages.push(lineageId)
    for (const locatorId of refs.locatorIds) if (!locatorIds.includes(locatorId)) locatorIds.push(locatorId)
    if (refs.lineageIds.length !== 1) addError(errors, `hypothesis:lineage_not_single_source:${item.id}`)
  }
  if (lineages.length > 1) addError(errors, 'hypothesis:cross_lineage_synthesis_forbidden')
  ...
  const referencedUnsafeIds = referenced.filter(item => item.kind === 'literature_claim' && evidenceState(item)).map(item => item.id)
  const referencedConflictEvidenceIds = referenced.filter(item => item.relation === 'conflicts' || item.status === 'conflict').map(item => item.id)
  ...
  for (const item of semanticBasis) if (evidenceState(item) || item.relation === 'conflicts' || item.status === 'conflict') addError(errors, `hypothesis:unsafe_semantic_basis:${item.id}`)
...
function makeContextEvidence(entry) {
  return {
    id: `astrology.user-context.${entry.id}`,
...
function buildConstitutionCheck(handoffPackage, submission, userContext) {
  ...
  const contextConflicts = contextEvidence
    ...
    .map(item => ({
      id: `astrology.user-context-conflict.${item.id.slice('astrology.user-context.'.length)}`,
      evidenceIds: [item.id],
  ...
  hypothesis.conflictIds = [...hypothesis.conflictIds, ...contextConflicts.map(conflict => conflict.id)]
...
function buildDecision({ hypothesis, userContext, evidenceInspection }) {
...
function readinessFor(decision, userContext) {
  ...
  return {
    ...
    blockers,
    reason: 'Submission validation permits only source-bounded hypothesis discussion; it never promotes runtime activation or definitive personal application.',
  }
}
...
function resultFor({ submission, userContext, evidenceInspection, constitution, decision }) {
  ...
  const userContextDecision = hasConflictingContext
    ...
    : hasSupportingContext
      ? 'reported_context_only_not_source_validation'
  ...
  return {
    ...
    status: decision === 'accepted_bounded_hypothesis'
      ...
      : decision === 'requires_user_confirmation'
        ? 'user_context_or_boundary_confirmation_required'
        : 'blocked',
    ...
    compositionEvidenceIds: evidenceInspection.semanticBasis.filter(item => item.evidenceRole === 'source_local_composition').map(item => item.id),
    sourceScope: clone(hypothesis.sourceScope),
    sourceTechnicalFactRefs: unique(evidenceInspection.sourceEvidence.flatMap(item => item.requiredTechnicalFactRefs || [])),
    ...
    hypothesisUse: decision === 'accepted_bounded_hypothesis' ? 'bounded_source_and_explicit_context_discussion_only' : 'do_not_apply_until_boundary_is_confirmed',
...
export function buildAstrologyWesternHypothesisSubmissionValidation({ handoffPackage, submission } = {}) {
  const handoffValidation = validateAstrologyWesternConversationalHandoffPackage(handoffPackage)
  ...
  if (submission.schemaVersion !== ASTROLOGY_WESTERN_HYPOTHESIS_SUBMISSION_SCHEMA || submission.version !== ASTROLOGY_WESTERN_HYPOTHESIS_SUBMISSION_VERSION || submission.kind !== ASTROLOGY_WESTERN_HYPOTHESIS_SUBMISSION_KIND) addError(errors, 'submission_schema_mismatch')
  ...
  const evidenceInspection = inspectEvidence({ handoffPackage, hypothesis: hypothesisShape }, errors)
  ...
  const decision = buildDecision({ hypothesis: submission.hypothesis, userContext, evidenceInspection })
  const result = resultFor({ submission, userContext, evidenceInspection, constitution, decision })
  const validation = {
    schemaVersion: ASTROLOGY_WESTERN_HYPOTHESIS_VALIDATION_SCHEMA,
    version: ASTROLOGY_WESTERN_HYPOTHESIS_VALIDATION_VERSION,
    kind: ASTROLOGY_WESTERN_HYPOTHESIS_VALIDATION_KIND,
    handoffPackage: {
...
export const exportAstrologyWesternHypothesisSubmissionJson = (submission, indent = 2) => JSON.stringify(submission, null, indent)

export function consumeAstrologyWesternHypothesisSubmission(serialized, { handoffPackage } = {}) {
  let submission
  try {
  ...
  return buildAstrologyWesternHypothesisSubmissionValidation({ handoffPackage, submission })
}
