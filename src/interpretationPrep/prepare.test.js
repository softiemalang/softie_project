...
test('conversation export filters by topic and keeps model instructions', () => {
  ...
  assert.equal(payload.calculationSummary.saju.policyContract.status, 'SELECTED')
  assert.equal(payload.calculationSummary.saju.policyContract.historicalAuthority, 'insufficient_evidence')
  assert.equal(payload.calculationSummary.saju.policyContract.historicalFact, false)
  assert.equal(payload.calculationSummary.saju.policyContract.implementationPolicy, true)
  assert.equal(payload.calculationSummary.saju.policyContract.readinessStatus, 'blocked')
  assert.ok(payload.instruction.includes('계산값을 임의로 변경하지 말고'))
  assert.ok(payload.instruction.includes('historicalFact=false'))
  ...
  assert.match(markdown, /## Saju 계산 정책 경계/)
  assert.match(markdown, /historicalAuthority=insufficient_evidence/)
  assert.match(markdown, /implementationPolicy=true/)
