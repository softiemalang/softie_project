export function buildInterpretationProfile({ natalAnalysis, dailyInteraction, gender }) {
  ...
  const { supplements, overloads, branchRelations, signals, fieldImpacts, periodContext, dayType, sectionPriority, longerCycleContext, dailyBalance } = dailyInteraction;
  ...
  const supportiveElementHints = supportiveElements?.reasonHints ?? []
  const balancingContextHints = [
    supportiveElements?.likelyHelpful?.length ? '안정감을 되찾는 데 도움이 되는 흐름이 함께 있음' : null,
    supportiveElements?.likelyOverloading?.length ? '이미 강한 흐름은 책임감이나 몸의 무게감으로 느껴질 수 있음' : null,
    supportiveElements?.cautionHints?.[0] || null,
  ].filter(Boolean)
  const dayTypeHint = dayType?.label || null
  const sectionPriorityHints = [
    sectionPriority?.primary?.length
      ? `오늘은 ${sectionPriority.primary.map((section) => sectionLabels[section]).join(', ')} 흐름을 우선해서 살피는 편이 좋음`
      : null,
    sectionPriority?.secondary?.length
  ...
  const dailyBalanceHint = dailyBalance?.balanceHint || null
  const dailyBalanceActionHint = dailyBalance?.actionFocus || null
  ...
  const dailyKeyPoints = [
    ...
    balancingContextHints[0] || null,
    dayTypeHint ? `오늘의 성격은 ${dayTypeHint}` : null,
    ...(dayType?.reasons || []),
    ...sectionPriorityHints,
    ...(todayFlowPositionHint ? [todayFlowPositionHint] : []),
    ...longerCycleHints,
    dailyBalanceHint || null,
    dailyBalance?.mainOpportunity || null,
    dailyBalance?.mainRecovery || null,
  ...
  const fieldReasonHints = {
    work: [
      ...
      supportiveElementHints.find((hint) => hint.includes('표현') || hint.includes('결과')) || null,
      periodContext?.year?.roleHints?.[0] ? `올해 흐름은 ${periodContext.year.roleHints[0]}` : null,
      periodContext?.month?.roleHints?.[0] ? `이번 달 흐름은 ${periodContext.month.roleHints[0]}` : null,
      ...
      dailyBalance?.mainOpportunity?.includes('결과물') || dailyBalance?.mainOpportunity?.includes('책임') ? dailyBalance.mainOpportunity : null,
    ].filter(Boolean),
    money: [
      ...
      dailyBalance?.mainCaution?.includes('지출') || dailyBalance?.mainOpportunity?.includes('지출') ? (dailyBalance.mainCaution || dailyBalance.mainOpportunity) : null,
    ].filter(Boolean),
    relationships: [
      ...
      dailyBalance?.mainCaution?.includes('반응 속도') || dailyBalance?.mainOpportunity?.includes('관계의 온도') ? (dailyBalance.mainCaution || dailyBalance.mainOpportunity) : null,
    ].filter(Boolean),
    love: [
      ...
      ...(sectionPriority?.reasonHints?.love || []),
      dailyBalance?.mainCaution?.includes('표현의 온도') || dailyBalance?.mainOpportunity?.includes('관계의 온도') ? (dailyBalance.mainCaution || dailyBalance.mainOpportunity) : null,
    ].filter(Boolean),
    health: [
    ...
    mind: [
      ...
      dailyBalance?.mainRecovery?.includes('생각') || dailyBalance?.mainRecovery?.includes('마음') ? dailyBalance.mainRecovery : null,
    ].filter(Boolean),
  };
  ...
  const basisHintParts = [
    ...
    periodContext?.day?.roleHints?.[0] ? `오늘은 ${periodContext.day.roleHints[0]}` : null,
    dayTypeHint ? `${dayTypeHint}이라는 결이 바탕에 깔려 있고` : null,
    ...
    dailyBalanceHint || null,
    dailyBalance?.reasonHints?.[0] || null,
