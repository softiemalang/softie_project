...
const errors = [
  ...
  ...(artifact.artifactByteSha256 !== createHash('sha256').update(canonicalSajuReadinessJson({ ...artifactPayloadWithoutIdentity(artifact), artifactByteSha256: null })).digest('hex') ? ['artifact byte hash preimage mismatch'] : []),
  ...checkArtifactIdentity(artifact, { root: new URL('../', import.meta.url).pathname, artifactId: 'saju-readiness-grounding-v0', materializerPath: 'scripts/materialize-saju-readiness-grounding-v0.mjs', materializerVersion: '1.1.0' }),
]
