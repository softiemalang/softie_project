...
import {
  buildSajuLunaP0EvidenceAcquisitionV2,
...
export const HISTORICAL_CHECKER_PATH = 'scripts/check-saju-luna-p0-evidence-acquisition-v2-historical.mjs'
...
async function checkPredecessor(root, artifactPath, stored) {
  ...
  const result = artifactPath.includes('gemini-witness-dossier-adjudication-v1')
    ...
    : await checkTypedHistoricalArtifact(stored.artifact, { artifactPath, root })
...
export async function checkHistoricalArtifact(
  ...
  errors.push(...checkSajuLunaP0EvidenceAcquisitionV2(candidate))
  ...
  if (candidate.artifactIdentity && PREDECESSOR_PATHS.every(path => predecessorStored[path].artifact)) {
    const replayPayload = buildSajuLunaP0EvidenceAcquisitionV2({
