...
export function checkArchiveScanSourceWitness(artifact, { pdfBytes = null, root = process.cwd() } = {}) {
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  const root = resolve(new URL('..', import.meta.url).pathname); const path = resolve(process.argv[2] || 'artifacts/ziwei-archive-scan-source-witness-admission-v0/complete.json'); const artifact = JSON.parse(await readFile(path,'utf8')); const pdfPath = process.argv[3]; const pdfBytes = pdfPath ? await readFile(pdfPath) : null; const failures = checkArchiveScanSourceWitness(artifact,{pdfBytes,root}); const bytes=await readFile(path)
