...
import { matchesFileByteIdentity } from '../src/artifactIdentity.js'
...
const packet=JSON.parse(readFileSync(resolve(dir,'complete.json'))), sha=b=>createHash('sha256').update(b).digest('hex')
...
for(const [p,h] of Object.entries(packet.protectedActualBytes)) {if(!existsSync(resolve(root,p)) && !packet.basisHead) fail(`protected missing ${p}`); else if(!matchesFileByteIdentity(root,p,h,{generationBaseHead:packet.basisHead})) fail(`protected hash drift ${p}`)}
