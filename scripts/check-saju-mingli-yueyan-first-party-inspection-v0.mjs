...
import {
  ...
  VERSION,
  buildArtifact,
} from './materialize-saju-mingli-yueyan-first-party-inspection-v0.mjs'
...
export async function checkArtifact(candidate, { root = ROOT, historical = false } = {}) {
  const errors = checkSajuMingliYueyanFirstPartyInspection(candidate)
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  ...
  console.log(JSON.stringify({
    ...
    firstPartyItemIdentityConfirmed: artifact.summary?.firstPartyItemIdentityConfirmed || false,
    firstPartyTargetPageObtained: artifact.summary?.firstPartyTargetPageObtained || false,
    directTargetPageObservationCount: artifact.summary?.directTargetPageObservationCount || 0,
    mirrorLocatorCount: artifact.summary?.mirrorLocatorCount || 0,
    ...
    canonicalTransmissionEdgeCount: artifact.digitalPhysicalRelationshipAudit?.canonicalTransmissionEdges?.length || 0,
    promotionCount: artifact.promotion?.stableClaimPromotionCount || 0,
