...
import {
  ...
  SCHEMA,
  VERSION,
} from './materialize-saju-gemini-witness-dossier-adjudication-v2.mjs'
import {
  checkHistoricalArtifact as checkDossierHistoricalArtifact,
} from './check-saju-gemini-witness-dossier-adjudication-v1-historical.mjs'
import {
  checkHistoricalArtifact as checkP0HistoricalArtifact,
} from './check-saju-luna-p0-evidence-acquisition-v2-historical.mjs'
import { verifyIntegritySidecar } from './check-saju-five-classics-typed-readiness-contract-v0-historical.mjs'

export const HISTORICAL_CHECKER_PATH = 'scripts/check-saju-gemini-witness-dossier-adjudication-v2-historical.mjs'
...
async function checkPredecessor(root, artifactPath, stored) {
  ...
  const result = artifactPath.includes('gemini-witness-dossier-adjudication-v1')
    ? await checkDossierHistoricalArtifact(stored.artifact, { artifactPath, root })
    : await checkP0HistoricalArtifact(stored.artifact, { artifactPath, root })
...
export async function checkHistoricalArtifact(
  ...
  errors.push(...checkSajuGeminiWitnessDossierAdjudicationV2(candidate))
  errors.push(...checkArtifactIdentity(candidate, {
    allowGenerationBaseInput: true,
    artifactId: SCHEMA,
    materializerPath: 'scripts/materialize-saju-gemini-witness-dossier-adjudication-v2.mjs',
  ...
  if (candidate.artifactIdentity && PREDECESSOR_PATHS.every(path => predecessorStored[path].artifact)) {
    const replayPayload = buildSajuGeminiWitnessDossierAdjudicationV2({
