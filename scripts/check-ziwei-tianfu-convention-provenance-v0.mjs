...
export async function checkArtifact(candidate, root = resolve(new URL('..', import.meta.url).pathname)) {
  const useHistoricalInputs = (candidate.artifactIdentity?.inputs ?? []).some(item => !matchesFileByteIdentity(root, item.path, item.byteSha256))
  ...
  for (const item of candidate.immutableExistingBytes ?? []) {
    if (!matchesFileByteIdentity(root, item.path, item.sha256, { generationBaseHead: candidate.artifactIdentity?.generation?.baseHead })) errors.push(`immutable_existing:${item.path}`)
  }
