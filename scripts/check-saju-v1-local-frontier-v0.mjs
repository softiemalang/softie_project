...
const errors = [
  ...
  ...checkArtifactIdentity(artifact, { root, artifactId: SCHEMA, materializerPath: 'scripts/materialize-saju-v1-local-frontier-v0.mjs', materializerVersion: VERSION }),
]
...
if (canonicalIdentityJson(artifact) !== canonicalIdentityJson(expected)) errors.push('materialized content drift')
...
console.log(JSON.stringify({ status: errors.length ? 'fail' : 'pass', head, claimCount: artifact.claims.length, occurrenceCount: artifact.scope.canonicalOccurrenceCount, taxonomy: artifact.taxonomy.distribution, errors }, null, 2))
