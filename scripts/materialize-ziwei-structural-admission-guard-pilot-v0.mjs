...
import { buildAdmissionUnit, GUARD_SCHEMA, PILOT_RESULTS } from './lib/ziwei-structural-admission-guard.mjs'
...
function buildRecord(audit, auditOccurrence, provenanceOccurrence) {
  ...
  return {
    ...
    admissionUnit: buildAdmissionUnit({ occurrence, guard }),
...
export async function buildStructuralAdmissionGuardPilot() {
  ...
  const provenance = JSON.parse(await readFile(resolve(root, PROVENANCE_PATH), 'utf8'))
  ...
  return attachArtifactIdentity({
    ...
    sourceProvenance: { schemaVersion: provenance.schemaVersion, path: PROVENANCE_PATH, occurrenceCount: provenance.occurrences.length, stableClaimBoundary: provenance.stableClaimBoundary.count, sourceIdentity: provenance.sourceIdentityInventorySummary },
