...
const extracted = archive + '.extracted'; await rm(extracted, { recursive: true, force: true }); execFileSync('unzip', ['-q', archive, '-d', extracted]); const manifest = JSON.parse(await readFile(`${extracted}/sample-asset-manifest.json`));
...
const sample = await readFile(`${extracted}/${expectedNames[0]}`); const item = manifest.contents[0]; const sampleSha = createHash('sha256').update(sample).digest('hex'); if (sampleSha !== item.sha256 || sample.length !== item.bytes || item.rowCount !== 150671 || sample.includes(13)) throw new Error('sample identity/schema normalization failed')
