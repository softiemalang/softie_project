...
import {
  ...
  SCHEMA,
  VERSION,
} from './materialize-saju-gemini-v6-parent-adjudication.mjs'
import {
...
export const HISTORICAL_CHECKER_PATH = 'scripts/check-saju-gemini-v6-parent-adjudication-historical.mjs'
...
async function checkPredecessor(root, artifactPath, stored) {
  ...
  if (artifactPath.includes('saju-gemini-witness-dossier-adjudication-v3')) {
...
export async function checkHistoricalArtifact(
  ...
  errors.push(...checkSajuGeminiV6ParentAdjudication(candidate))
  ...
  if (candidate.artifactIdentity && typedArtifact && PREDECESSOR_PATHS.every(path => predecessorStored[path].artifact)) {
    const replayPayload = buildSajuGeminiV6ParentAdjudication({
