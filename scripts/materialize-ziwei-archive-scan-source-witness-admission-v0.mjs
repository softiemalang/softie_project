...
export async function materializeArchiveScanSourceWitness() {
  const root = resolve(new URL('..', import.meta.url).pathname)
  const observedHead = execFileSync('git', ['-c','core.fsmonitor=false','rev-parse','HEAD'], { cwd: root, encoding: 'utf8' }).trim()
  if (observedHead !== BASIS_HEAD) throw new Error(`admission requires HEAD ${BASIS_HEAD}; observed ${observedHead}`)
  ...
  return attachArtifactIdentity(artifact, buildArtifactIdentity({ root, artifactId: SCHEMA, materializerPath: artifact.materializer, materializerVersion: VERSION, baseHead: observedHead, inputs: [INPUT] }))
}
