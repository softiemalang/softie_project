...
} from '../scripts/check-saju-gemini-witness-dossier-adjudication-v2-historical.mjs'
import {
  ARTIFACT_PATH,
  PREDECESSOR_PATHS,
} from '../scripts/materialize-saju-gemini-witness-dossier-adjudication-v2.mjs'
...
test('Dossier v2 historical replay preserves stored bytes and both predecessor boundaries', async () => {
...
test('Dossier v2 historical verification rejects snapshot and predecessor sidecar tampering', async () => {
...
test('Dossier v2 historical verification rejects predecessor identity drift', async () => {
  const artifact = await readJson(ARTIFACT_PATH)
  artifact.predecessorReferences[PREDECESSOR_PATHS[1]].artifactPayloadSha256 = '0'.repeat(64)
  const result = await checkHistoricalArtifact(artifact)
  assert.ok(result.errors.includes('candidate_file_mismatch'))
  assert.ok(result.errors.includes(`predecessor_identity:${PREDECESSOR_PATHS[1]}`))
  assert.ok(result.errors.includes('historical_replay_content'))
})

test('Dossier v2 historical checker CLI reports replay without external PDF access', () => {
