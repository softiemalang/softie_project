...
test('async enter recipe is a single 200ms opacity-only rule with an explicit reduced-motion stop', () => {
  ...
  const recipe = styles.match(
    /\/\* First successful Today event fetch only; data is rendered immediately and opacity is the only animated property\. \*\/[\s\S]*?@media \(prefers-reduced-motion: reduce\) \{[\s\S]*?\n\}/,
  )?.[0]
  ...
  assert.match(recipe, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?animation:\s*none;/)
})
