...
test('legacy analyzer and materializer are deterministic and separate control mismatch from arithmetic', async () => {
  ...
  for (const [name, value] of [['ubuntu-gcc', '0x3ff0000000000000'], ['alpine-gcc', '0x3ff0000000000001'], ['alpine-clang', '0x3ff0000000000002']]) await writeFile(join(fixture, `${name}.jsonl`), row('s0', [value, '0x0', '0x0', '0x0', '0x0', '0x0']))
  const manifest = { schemaVersion: 1, recordType: 'de405_legacy_matrix_manifest', baselineHead: '33e8215f1349860e6166f7d1c779b6d36b6a9624', root: fixture, referenceVariant: 'ubuntu-gcc', controlTaxonomy: { requiredIdentical: {} }, variants: ['ubuntu-gcc', 'alpine-gcc', 'alpine-clang'].map(id => ({ id, output: { path: `${id}.jsonl` } })) }; const manifestPath = join(base, 'manifest.json'); await writeFile(manifestPath, JSON.stringify(manifest))
...
test('legacy analyzer fail-closes userspace/compiler control mismatch while retaining arithmetic comparison', async () => {
  ...
  for (const [name, libc] of [['ubuntu-gcc', 'glibc'], ['alpine-gcc', 'musl']]) { await writeFile(join(fixture, `${name}.jsonl`), row('s0', ['0x3ff0000000000000', '0x0', '0x0', '0x0', '0x0', '0x0'])); await writeFile(join(fixture, `${name}.provenance.json`), JSON.stringify({ userspace: { libc }, controls: { flags: ['same'] } })) }
  const manifest = { schemaVersion: 1, recordType: 'de405_legacy_matrix_manifest', baselineHead: '33e8215f1349860e6166f7d1c779b6d36b6a9624', root: fixture, referenceVariant: 'ubuntu-gcc', controlTaxonomy: { requiredIdentical: ['userspace.libc', 'controls.flags'] }, variants: ['ubuntu-gcc', 'alpine-gcc'].map(id => ({ id, output: { path: `${id}.jsonl` } })) }; const manifestPath = join(base, 'manifest.json'); const output = join(base, 'analysis.json'); await writeFile(manifestPath, JSON.stringify(manifest)); await run('node', ['scripts/analyze-de405-legacy-matrix.mjs', '--manifest', manifestPath, '--output', output], { cwd: root }); const analysis = JSON.parse(await readFile(output)); assert.equal(analysis.comparisons[0].classification, 'blocked_userspace_compiler_source_input_mismatch'); assert.equal(analysis.comparisons[0].differingComponents, 0)
})
