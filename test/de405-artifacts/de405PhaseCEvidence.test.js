...
test('Phase D candidate evidence contains one official selected JPL candidate per unresolved sample', async () => {
  const rows = await readJsonLines(`${artifactRoot}/de405-jpl-cspice-candidate-state-evidence.jsonl`)
...
test('Phase D out-of-coverage investigation is empty after manifest regeneration', async () => {
  const report = JSON.parse(await readFile(`${artifactRoot}/de405-jpl-cspice-out-of-coverage-investigation.json`, 'utf8'))
