...
import {
  ...
  buildBundle,
  materializeBundle,
} from '../scripts/materialize-ziwei-p0-palace-branch-slot-composition-v5.mjs'
import { checkArtifact } from '../scripts/check-ziwei-p0-palace-branch-slot-composition-v5.mjs'

test('v5 records NARA chart-example and 1871 catalog frontiers without graph or readiness promotion', async () => {
  ...
  assert.equal(first.artifact.scope.heldOutResearchCandidateCount, 5)
  assert.equal(first.artifact.scope.researchCandidatesAdmitted, 0)
  assert.equal(first.artifact.researchFrontier.candidates.length, 5)
  assert.equal(first.artifact.researchFrontier.candidates.find(item => item.candidateId.includes('nara-')).locators.reviewedImages.length, 26)
  assert.equal(first.artifact.researchFrontier.candidates.find(item => item.candidateId.includes('ritsumeikan-')).directVisualReview, false)
  ...
  const directory = await mkdtemp(join(tmpdir(), 'ziwei-palace-composition-v5-check-'))
  try {
...
test('v5 materialization is byte-identical on repeated writes', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'ziwei-palace-composition-v5-deterministic-'))
  try {
    const target = join(directory, 'complete.json')
...
test('v5 negative checker rejects frontier, byte-identity, graph, readiness, authority, and timestamp shortcuts', () => {
  const output = execFileSync(
    'node',
    ['scripts/check-ziwei-p0-palace-branch-slot-composition-v5-negative-v0.mjs'],
