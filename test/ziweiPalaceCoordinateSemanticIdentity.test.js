...
import { buildArtifact, canonicalJson, SCHEMA } from '../scripts/materialize-ziwei-palace-coordinate-semantic-identity-v0.mjs'
...
test('palace coordinate packet materialization is byte deterministic', async () => {
  ...
  try {
    ...
    for (const target of [a, b]) assert.equal(spawnSync(process.execPath, [`scripts/materialize-${SCHEMA}.mjs`, target], { cwd: process.cwd(), encoding: 'utf8' }).status, 0)
