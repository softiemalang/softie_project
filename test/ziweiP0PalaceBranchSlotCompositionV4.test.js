...
import {
  ...
  buildBundle,
  materializeBundle,
} from '../scripts/materialize-ziwei-p0-palace-branch-slot-composition-v4.mjs'
import { checkArtifact } from '../scripts/check-ziwei-p0-palace-branch-slot-composition-v4.mjs'

test('v4 records the expanded negative frontier without graph or readiness promotion', async () => {
  ...
  assert.equal(first.artifact.scope.heldOutResearchCandidateCount, 3)
  ...
  assert.equal(first.artifact.researchFrontier.candidates[0].doesNotEnterGraph, true)
  assert.equal(first.artifact.researchFrontier.candidates[1].doesNotEnterGraph, true)
  assert.equal(first.artifact.researchFrontier.candidates[2].doesNotEnterGraph, true)
  ...
  const directory = await mkdtemp(join(tmpdir(), 'ziwei-palace-composition-v4-check-'))
  try {
...
test('v4 materialization is byte-identical on repeated writes', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'ziwei-palace-composition-v4-deterministic-'))
  try {
    const target = join(directory, 'complete.json')
...
test('v4 negative checker rejects research-frontier admission, lineage, graph, readiness, and authority shortcuts', () => {
  const output = execFileSync(
    'node',
    ['scripts/check-ziwei-p0-palace-branch-slot-composition-v4-negative-v0.mjs'],
