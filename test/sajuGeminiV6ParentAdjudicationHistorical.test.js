...
} from '../scripts/check-saju-gemini-v6-parent-adjudication-historical.mjs'
import {
  ARTIFACT_PATH,
  PREDECESSOR_PATHS,
} from '../scripts/materialize-saju-gemini-v6-parent-adjudication.mjs'
...
test('Gemini v6 historical replay preserves the stored parent overlay and four predecessor identities', async () => {
  ...
  assert.equal(result.predecessors['artifacts/saju-five-classics-research-continuation-v1/complete.json'].historicalReplay, true)
...
test('Gemini v6 historical verification rejects snapshot and predecessor sidecar tampering', async () => {
...
test('Gemini v6 historical verification rejects predecessor identity drift', async () => {
  const artifact = await readJson(ARTIFACT_PATH)
  artifact.predecessor.references[PREDECESSOR_PATHS[2]].artifactPayloadSha256 = '0'.repeat(64)
  const result = await checkHistoricalArtifact(artifact)
  assert.ok(result.errors.includes('candidate_file_mismatch'))
  assert.ok(result.errors.includes(`predecessor_identity:${PREDECESSOR_PATHS[2]}`))
  assert.ok(result.errors.includes('historical_replay_content'))
})

test('Gemini v6 historical checker reports replay without external PDF or candidate packet access', () => {
