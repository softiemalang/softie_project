...
test('frontend exposes only the simple Chat handoff flow and does not import Lab session UI', () => {
  ...
  assert.match(cardSource, /exportDeterministicBaseJson/)
  assert.doesNotMatch(cardSource, /JSON 다운로드/)
  assert.doesNotMatch(cardSource, /downloadJson/)
  assert.match(cardSource, /canonical Base 복사/)
  assert.match(cardSource, /Markdown 다운로드/)
  assert.doesNotMatch(pageSource, /localStorage|TOPIC_PRESETS|questionText|prep-save-toggle/)
  assert.doesNotMatch(cardSource, /TOPIC_PRESETS|questionText|privacyMinimal|gyeokguk|yongShin|shinsal/)
})
