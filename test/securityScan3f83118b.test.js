...
test('public Saju and internal test surfaces are bounded or fail closed', () => {
  ...
  assert.match(generate, /SAJU_POLICY_VALUES/)
  assert.match(generate, /policyContract: getSajuPolicyBoundary\(computedData\?\.policyContract\)/)
  assert.match(generate, /return jsonResponse\(buildPublicReportResponse\(savedReport, false\)\)/)
  ...
  assert.match(api, /normalizeSajuSnapshotForConsumer\(data\)/)
  assert.match(api, /normalizeSajuReportForConsumer\(data\)/)
